...
class MenuController extends ManageController implements CurdInterface
{
    ...
    {
        $group = Yii::$app->request->get('group',0);
    ...
    public function actionDelete($id)
    {
        ...
        // 获取子菜单
        $ids = ArrayHelper::getChildesId($this->getMenu($model->group),$id);
        $ids[] = $id;
    ...
    protected function getMenu($group){
        $list = ArrayHelper::linear($this->findModel()->find()->where(['group'=>$group])->asArray()->all(),' ├ ');
