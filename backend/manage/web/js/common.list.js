...
var listApp = function () {
      ...
      switch ($this.data('action')){
        ...
        case 'status':
          ...
          commonApp.fieldUpdateRequest($this,{
            ...
            successCallback:function(){
              if(_status==1){
                $this.data('value',0).html('<span class="iconfont">&#xe62a;</span>');
              }else{
                $this.data('value',1).html('<span class="iconfont">&#xe625;</span>');
              }
            }
          });
          break;
          ...
          commonApp.fieldUpdateRequest($this,{
            ...
            successCallback:function(){
              for(var i=0;i<_id.length;i++){
                if(_status == 1){
                  $('.status-'+ _id[i]).data('value',0).html('<span class="iconfont">&#xe62a;</span>');
                }else{
                  $('.status-'+ _id[i]).data('value',1).html('<span class="iconfont">&#xe625;</span>');
                }
              }
              commonApp.chooseData.all($DATA_CHECKBOX,false);
            }
  ...
  var _sortFun = function(){
    ...
    $('#j_sort_batch').click(function () {
      ...
      commonApp.dialog.default(['批量排序',$('#tpl_sort_batch').html()],{
        onShow:function ($dialog) {
          $dialog.find('.dd').nestable({'maxDepth':$this.data('depth')||10});
        },
        confirm:function () {
          var $dd = $(this).find('.dd'),
              newData = _nestableHandel($dd.nestable('serialize'),$this.data('pid')),
    ...
    // nestable返回的数据处理
    function _nestableHandel(data,pid) {
      ...
      $.each(data,function (i,n) {
        if(typeof n.children == 'undefined'){
          ...
          n.pid = pid;
          _res.push(n);
          _res = _res.concat(_nestableHandel(child,n.id));
        }
      });
      return _res;
    }
