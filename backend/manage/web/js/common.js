...
var commonApp = function () {
  ...
  var _initFun = function () {
    ...
    inFrameFun(function(){
      ...
      // 记录当前页面到本地
      parent.indexApp.setHistory(location.href);
    ...
    $('.j_date_piker').each(function(i,n){
      var $this = $(this),
        currVal = $this.val(),
        $input = $('<input type="text" name="date_piker_'+ i +'" class="form-control">');
  ...
  var getUrlParamFun = function (name) {
   ...
   * @returns {string}
   */
  var setUrlParamFun = function (name,value){
    ...
    if(_search){
      ...
      if(!_exit) _url = _url = _url+'&'+name+'='+value;
    }else{
      _url = _url = _url+(_url.indexOf('?')>-1?'':'?')+name+'='+value;
    }
    return _url;
  };
  ...
  var fieldUpdateRequestFun = function ($e,config){
    var _setting = {
      ...
      success: function (result) {
        var _html = '';
        if(result.status == 1){
          commonApp.notify.success(this.successTxt);
