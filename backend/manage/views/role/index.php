...
<?= Html::a('新增角色', ['create'], ['class' => 'btn btn-primary']) ?>
<?php $this->endBlock(); ?>
            ...
            <table class="table table-hover" id="list_data">
                ...
                <?php foreach($dataList = $dataProvider->models as $item){ ?>
                    <tr>
                        ...
                        <td><?=$item->id?></td>
                        <td><a href="<?=Url::to(['update','id'=>$item->id]);?>" class="text-primary"><?=Html::encode($item->name)?></a></td>
                        <td><?=Html::encode($item->remark)?></td>
                        <td class="opt" align="center">
                            <?= Html::a(Yii::t('common','Configure permissions'), ['access', 'id' => $item->id], ['class' => 'text-primary j_access']) ?>
                            <span>|</span>
