...
<?= Html::a('新增幻灯片', ['create','category_id'=>$categoryInfo->id], ['class' => 'btn btn-primary']) ?>
<?php $this->endBlock(); ?>
            ...
            <table class="table table-hover" id="list_data">
                ...
                <?php foreach($dataList = $dataProvider->models as $i=>$item){ ?>
                    <tr>
                        ...
                        <td><?=$item->id?></td>
                        <td>
                            <?php if(!empty($item->thumb)){?><img src="<?php $thumb = UrlHelper::fileDataHandle($item->thumb,false); echo $thumb['file']?>" height="50"><?php }else{ echo '———';}?>
                        </td>
                        <td><a href="<?=Url::to(['update','id'=>$item->id]);?>" class="text-primary"><?=strip_tags($item->title)?></a></td>
                        <td align="center">
