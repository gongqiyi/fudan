...
<div class="panel panel-default form-data">
    <div class="panel-body">
        ...
        <div class="tab-pane active" id="tab-category-base" role="tabpanel">
            ...
            <?= $form->field($model, 'model_id')->dropDownList(ArrayHelper::map($modelList,'id','title'), ['class'=>'form-control','prety'=>true,'prompt'=>'请选择所属模型'])?>
        ...
        <div class="tab-pane" id="tab-category-config" role="tabpanel">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-17">
                    <label class="radio-inline">
                        <input type="radio" name="PrototypeCategoryModel[expand][enable_detail]" value="1"<?=ArrayHelper::getValue($model->expand,'enable_detail')?' checked':''?>> 启用
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="PrototypeCategoryModel[expand][enable_detail]" value="0"<?=!ArrayHelper::getValue($model->expand,'enable_detail')?' checked':''?>> 禁用
                    </label>
                </div>
            </div>
            <?php
            ...
            ?>
            <?= $form->field($model, 'template')->textInput(['placeholder'=>'默认为：index'])?>
            <?= $form->field($model, 'template_content')->textInput(['placeholder'=>'默认为：detail'])?>
        </div>
