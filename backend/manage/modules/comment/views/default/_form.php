...
<div class="panel panel-default form-data">
    <div class="panel-body">
        ...
        <!-- 表单控件开始 -->
        <?= $form->field($model, 'content')->textarea(['disabled'=>true,'rows'=>10])?>
        <?php if(!empty($model->atlas)){?>
        <div class="form-group">
            <label class="col-sm-4 control-label">图片</label>
            <div class="col-sm-17">
                <div class="list-img list-img-more clearfix j_uploadimg" data-name="uploadimg">
                    <ul>
                        <?php
                        $photo = UploadDataHelper::dataHandle($model->atlas,true);
                        foreach($photo as $item){
                        ?>
                        <li>
                            <div class="pic">
                                <div class="inner">
                                    <a href="<?=UploadDataHelper::getItem($item,'file')?>" target="_blank"><img src="<?=UploadDataHelper::getItem($item,'file')?>"></a>
                                </div>
                            </div>
                        </li>
                        <?php }?>
